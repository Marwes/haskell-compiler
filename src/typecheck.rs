...
impl<'a> TypeEnvironment<'a> {
        ...
        for instance in module.instances.iter_mut() {
            ...
            }
            {
                let mut missing_super_classes = self
                    ...
                    .filter(|constraint| {
                        self.has_instance(constraint.class, &instance.typ, &mut vec![])
                            .is_err()
                    })
                    .peekable();
    ...
    fn typecheck_binding_group(&mut self, subs: &mut Substitution, bindings: &mut [Binding<Name>]) {
        debug!(
            "Begin typecheck {:?} :: {:?}",
            bindings[0].name, bindings[0].typ
        );
        let mut argument_types: Vec<_> = (0..bindings[0].arguments.len())
            .map(|_| self.new_var())
            .collect();
        let type_var =
            match bindings[0].typ.value {
...
pub mod test {
    ...
    fn typecheck_lambda() {
        ...
        let e = lambda(
            "x",
            apply(apply(identifier("primIntAdd"), identifier("x")), number(1)),
        );
        let mut expr = rename_expr(e);
        env.typecheck_expr_(&mut expr);
...
",
        ...
        assert_eq!(
            ...
            qualified(
                vec![],
                function_type_(int_type(), Type::new_op(intern("Even"), vec![]))
            )
        );
    }
