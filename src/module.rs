...
impl TypedExpr {
    pub fn new<T: fmt::Display + AsRef<str>>(expr: Expr<T>) -> TypedExpr<T> {
        TypedExpr::with_location(expr, Location::eof())
    }
...
pub fn walk_module_mut<Ident, V: MutVisitor<Ident>>(visitor: &mut V, module: &mut Module<Ident>) {
    for bind in module
        .instances
        .iter_mut()
        .flat_map(|i| i.bindings.iter_mut())
    {
        visitor.visit_binding(bind);
    }
...
pub fn walk_binding_mut<Ident, V: MutVisitor<Ident>>(
    visitor: &mut V,
    binding: &mut Binding<Ident>,
) {
    match binding.matches {
...
pub fn walk_alternative_mut<Ident, V: MutVisitor<Ident>>(
    visitor: &mut V,
...
pub fn walk_pattern_mut<Ident, V: MutVisitor<Ident>>(
    visitor: &mut V,
...
pub fn encode_binding_identifier(
